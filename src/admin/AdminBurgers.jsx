    ...
    <div className="container-admin">
      ...
      <section>
        <div className="container-card d-flex flex-wrap justify-content-around align-items-space-around p-5 pb-5" >
          {burgers.map((burger) => (
            <div key={burger.id} className="card mt-3 mb-3" style={{ width: "15rem" }}>
              <img className="card-img-top" src={burger.picture} alt={burger.name} />
              ...
              <div className="card-body">
                <Link to={"/admin/burgers/update"} className="card-link">Update</Link>
                <button className='card-link delete' onClick={() => handleDelete(burger.id)}>Delete</button>
              </div>
            </div>
          ))}
        </div>
